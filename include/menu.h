...
enum menu_type {
   ...
   * The integer type menu has a max, min and a step value. Each step is
   *calculated. Will return the index of the selected value. Example: User goes
   *forwards twice then it will return 2.
   **/
  INT_TYPE,
  /**
   * @brief Menu type allowing user to select a float
   * The float type menu has a max, min and a step value. Each step is
   *calculated. Will return the index of the selected value. Example: User goes
   *forwards twice then it will return 2.
   **/
  FLOAT_TYPE,
...
typedef struct menu_t {
   ...
   * Step is how much the int menu will increase of decrease with each press.
   *Defaults to one
   * @author Chris Jerrett
   * @date 9/8/17
   **/
